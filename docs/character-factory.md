...
см. `pixel_clean.py`) → квантование до `--colors` (по умолчанию 28,
спека проекта — не больше 32). Квантование — обязательный шаг, если
...
python3 scripts/flatten_to_sprite.py path/to/ai-export.png \
  -o characters/<id>/reference/<name>.png --downscale 6 --colors 28
```

Компромисс: агрессивное квантование иногда даёт лёгкую постеризацию
(видимые полосы на плавных участках кожи) — на практике для Kat это
выглядело даже лучше «мыльного» варианта, но проверяйте глазами каждый раз,
не гоните через CLI вслепую.

`characters/kat/reference/kat-sporty-outfit-28c.png` — пример такого
результата (спортивно-готический образ, 170×256, 28 цветов).
