...
def quantize_visible_rgba(img: Image.Image, colours: int) -> Image.Image:
    """Quantize RGB without dithering, then restore binary alpha.

    Transparent pixels are ignored visually after alpha restoration. Their RGB
    value may consume one palette entry, so the visible palette is always <= the
    requested limit, never above it.
    """

    rgba = img.convert("RGBA")
    alpha = rgba.getchannel("A")

    # Replace transparent RGB with a deterministic colour before quantization.
...
def main() -> int:
    ...
    parser.add_argument(
        "--key",
        choices=("none", "black", "white", "custom"),
        default="black",
    ...
    # Quantization must never reintroduce partial transparency.
    ...
    print(
        ...
        f"visible_height={visible_h}px, visible_colours={visible_colour_count(sprite)}, "
        "alpha=binary, resampling=nearest"
    )
    return 0
