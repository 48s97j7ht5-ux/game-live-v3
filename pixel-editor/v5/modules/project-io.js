...
function decodeImage(source){
  return new Promise((resolve,reject)=>{
    ...
    image.onerror=()=>reject(new Error('не удалось прочитать PNG внутри проекта'));
    image.src=source;
  });
}
...
function downloadProject(project){
  ...
  const link=document.createElement('a');link.download='pixel-lab-'+stamp+'.pixelproject';link.href=url;link.click();
...
export default{
  ...
  mount(app){
    ...
    function serialize(){
      const active=app.layers.active();
      const source=app.referenceMagic?.sourceCanvas?.()||document.getElementById('ref');
      const reference=hasPixels(source)?{
        png:source.toDataURL('image/png'),
        visible:app.referenceVisibility?.visible!==false,
        raw:app.referenceDisplay?.raw===true,
        dimAlpha:app.referenceDisplay?.dimAlpha??.25,
        magic:app.referenceMagic?.enabled===true
      }:null;
      return{
        format:FORMAT,version:VERSION,width:W,height:H,savedAt:new Date().toISOString(),
        state:{
          activeLayerId:active?.id||null,
          color:app.state.color,
          activeTool:app.state.activeTool,
          cx:app.state.cx,cy:app.state.cy,
          loupeSize:+document.getElementById('loupeSize')?.value||15,
          tapOnly:document.getElementById('tapOnly')?.checked!==false,
    ...
    async function loadProject(project){
      ...
      const activeIndex=restoredLayers.findIndex(layer=>layer.id===activeId);
      app.state.activeLayer=activeIndex>=0?activeIndex:0;
      app.state.color=validColor(project.state?.color)?project.state.color:'#090404';
      app.state.cx=clamp(Number.isFinite(project.state?.cx)?Math.round(project.state.cx):67,0,W-1);
      app.state.cy=clamp(Number.isFinite(project.state?.cy)?Math.round(project.state.cy):200,0,H-1);
      app.state.undo=[];
      app.layers.validateStructure({repair:true,notify:true});

      const loupeSize=document.getElementById('loupeSize');
      if(loupeSize&&[...loupeSize.options].some(option=>+option.value===+project.state?.loupeSize))loupeSize.value=String(project.state.loupeSize);
      const tapOnly=document.getElementById('tapOnly');if(tapOnly)tapOnly.checked=project.state?.tapOnly!==false;
      if(validColor(app.state.color))app.emit('color:changed',app.state.color);
      if(app.tools.has(project.state?.activeTool))app.setTool(project.state.activeTool);

      app.referenceVisibility?.setVisible(project.reference?.visible!==false);
      app.referenceDisplay?.setDimAlpha(project.reference?.dimAlpha??.25);
      app.referenceDisplay?.setRaw(project.reference?.raw===true);
      app.backgroundToggle?.setMode(project.state?.background);
      if(referenceImage)app.referenceMagic?.loadSource?.(referenceImage,project.reference?.magic===true);
      else app.referenceMagic?.clearSource?.();

      app.emit('layers:changed');app.emit('composite:dirty');app.loupe?.draw();
      app.mobileLayout?.closeSheet?.();
      app.emit('status',`Проект открыт · ${restoredLayers.length} слоёв · ${W}×${H}`);
      return true;
    }

    async function loadFile(file){
      if(!file)return false;
      if(file.size>MAX_PROJECT_BYTES)throw new Error('файл проекта больше 50 МБ');
      let project;try{project=JSON.parse(await file.text())}catch{throw new Error('не удалось прочитать файл проекта')}
      return loadProject(project);
    }

    function save(){
      app.layers.validateStructure({repair:true,notify:true});
      const project=serialize();downloadProject(project);app.mobileLayout?.closeSheet?.();
      app.emit('status',`Проект сохранён · ${project.layers.length} слоёв · ${W}×${H}`);
      return project;
    }
    ...
    if(input)input.onchange=async event=>{
      ...
      try{await loadFile(file)}catch(error){app.emit('status','Ошибка проекта: '+error.message)}
    };
    if(openButton)openButton.onclick=openPicker;
    if(saveButton)saveButton.onclick=save;
    app.projectIO={serialize,save,openPicker,loadFile,loadProject,format:FORMAT,version:VERSION};
  }
};
