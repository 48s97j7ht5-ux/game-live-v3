export default{
  ...
  mount(app){
    ...
    const viewCard=document.getElementById('viewModes')?.closest('.card');
    ...
    const dock=document.createElement('div');dock.className='mobileDock';dock.innerHTML='<button data-tool="pencil">✏️<small>Pixel</small></button><button data-tool="eraser">⌫<small>Erase</small></button><button data-tool="picker">🎯<small>Pick</small></button><button data-mob="layers">▤<small>Layers</small></button><button data-mob="palette">●<small>Color</small></button><button data-mob="preview">👁<small>Preview</small></button>';
    ...
    function refreshTop(){top.querySelector('.mobileLayerName').textContent=activeName();top.querySelector('.mobileColor').style.background=app.state.color||'#090404'}
    ...
    dock.querySelector('[data-mob="layers"]').onclick=()=>openSheet(layersCard);
