export default{
  ...
  mount(app){
    ...
    function refreshRaw(){const c=quick.querySelector('[data-quick="refRaw"]');if(c)c.checked=app.referenceDisplay?.raw===true}
    ...
    function setWorkspace(mode){
      ...
      refreshTop();refreshBg();refreshRef();refreshMagic();refreshRaw();app.emit('mobile:workspace',mode)
    }
    function applyMode(){const on=mq.matches;body.classList.toggle('pixelLabMobile',on);top.hidden=!on;quick.hidden=!on;dock.hidden=!on;scrim.hidden=!on;if(on){setWorkspace('loupe');refreshTop();markTools();refreshBg();refreshRef();refreshMagic();refreshRaw()}else{closeSheet();delete body.dataset.mobileWorkspace;app.state.viewMode='composite';app.emit('composite:dirty');app.emit('mobile:workspace','desktop')}}
    ...
    app.on('layers:active',refreshTop);app.on('layers:changed',refreshTop);app.on('background:changed',refreshBg);app.on('reference:visibility',refreshRef);app.on('reference:magic',refreshMagic);app.on('reference:display',refreshRaw);
